...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private OnReadyForPullDownListener onReadyForPullDownListener;

	private SmoothScrollRunnable currentSmoothScrollRunnable;

	public final static int INDEX_LAYOUT = 1;

	public final static int ARTICLE_LAYOUT = 2;

	// ===========================================================
	// Constructors
	// ===========================================================

	public PullToRefreshBase(Context context, int type) {
		super(context);
		init(context, null, type);
	}

	public void init(Context context, AttributeSet attrs, int type) {
		if (type == INDEX_LAYOUT) {
			initIndex(context, attrs);
		} else if (type == ARTICLE_LAYOUT) {
			initArticle(context, attrs);
		} else {

		}
	}

	public PullToRefreshBase(Context context, int mode, int type) {
		super(context);
		this.mode = mode;
		init(context, null, type);
	}

	public PullToRefreshBase(Context context, AttributeSet attrs, int type) {
		super(context, attrs);
	...
	public final void setOnReadyForPullDownListener(OnReadyForPullDownListener listener){
		onReadyForPullDownListener = listener;
	}
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			// "isBeingDragged:" + String.valueOf(isBeingDragged) + ","
			// + lastMotionY);
			if (isBeingDragged) {
...
//					onRefreshListener.onRefresh();
					...
					}, SmoothScrollRunnable.ANIMATION_DURATION_MS);
				} else {
					smoothScrollTo(0);
				}
				return true;
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
//			Log.d("PullToRefreshBase",
//					"ACTION_MOVE:" + String.valueOf(isReadyForPullUp()));
			if (isReadyForPull()) {
	...
	private void initIndex(Context context, AttributeSet attrs) {
...
//		}
		...
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			...
			headerLayout = new LoadingHeaderLayout(context,
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			...
			footerLayout = new LoadingFooterLayout(context,
...
//		a.recycle();
	...
	private void initArticle(Context context, AttributeSet attrs) {
		setOrientation(LinearLayout.VERTICAL);
...
//		}
		...
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			// Loading View Strings
			String pullLabel = context
					.getString(R.string.pull_to_refresh_article_pull_down_label);
			String refreshingLabel = context
					.getString(R.string.pull_to_refresh_refreshing_label_meila);
			String releaseLabel = context
					.getString(R.string.pull_to_refresh_article_release_down_label);

			headerLayout = new LoadingArticleHeaderLayout(context,
					MODE_PULL_DOWN_TO_REFRESH, releaseLabel, pullLabel,
					refreshingLabel);
			addView(headerLayout, 0, new LinearLayout.LayoutParams(
					ViewGroup.LayoutParams.FILL_PARENT,
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			// // Loading View Strings
			String pullLabel = context
					.getString(R.string.pull_to_refresh_article_pull_up_label);
			String refreshingLabel = context
					.getString(R.string.pull_to_refresh_article_refreshing_label);
			String releaseLabel = context
					.getString(R.string.pull_to_refresh_article_release_up_label);

			footerLayout = new LoadingArticleFooterLayout(context,
					MODE_PULL_UP_TO_REFRESH, releaseLabel, pullLabel,
					refreshingLabel);
			addView(footerLayout, new LinearLayout.LayoutParams(
					ViewGroup.LayoutParams.FILL_PARENT,
...
//				newHeight = 0;
		...
		if(onReadyForPullDownListener != null){
//			Log.d("PullToRefreshBase", "onReadyForPullDownListener");
			onReadyForPullDownListener.onReadyForPullDown();
		}
	...
	public static interface OnReadyForPullDownListener{
		public void onReadyForPullDown();
	}

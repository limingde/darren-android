...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView>
	...
	public PullToRefreshAdapterViewBase(Context context) {
		super(context, PullToRefreshBase.INDEX_LAYOUT);
		refreshableView.setOnScrollListener(this);
		setHeaderFooterDivider(false);
	}

	public PullToRefreshAdapterViewBase(Context context, int mode) {
		super(context, mode, PullToRefreshBase.INDEX_LAYOUT);
		refreshableView.setOnScrollListener(this);
		setHeaderFooterDivider(false);
	}

	public PullToRefreshAdapterViewBase(Context context, AttributeSet attrs) {
		super(context, attrs, PullToRefreshBase.INDEX_LAYOUT);
		refreshableView.setOnScrollListener(this);
		setHeaderFooterDivider(false);
	}
	
	private void setHeaderFooterDivider(boolean dividerEnabled) {
		if (refreshableView != null && refreshableView instanceof ListView) {
			ListView listView = (ListView) refreshableView;
			listView.setHeaderDividersEnabled(dividerEnabled);
			listView.setFooterDividersEnabled(dividerEnabled);
		}
	}
	...
	private boolean isLastItemVisible() {
		...
		int z = topOffset > 0 ? 2 : 1;
//		Log.d("isLastItemVisible", "count:" + count + ",lastVisiblePosition"
//				+ refreshableView.getLastVisiblePosition() + ",z:" + z);
		if (count == 0) {
			return true;
		} else if (lastVisiblePosition == count - z) {
...
//			Log.d("isLastItemVisible",
//					"lastVisibleChild.getBottom():"
//							+ lastVisibleChild.getBottom()
//							+ ",refreshableView.getBottom()"
//							+ refreshableView.getBottom());

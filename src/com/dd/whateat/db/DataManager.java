...
public abstract class DataManager {
	 ...
	 *             数据库访问异常
	 */
	public void insert(BaseModel model) throws DataAccessException {
		if (model == null) {
...
//					values.put(Utils.toSQLName(column.getName()), String.valueOf(fieldValue));
		...
		//这里之前 是： for(int i=0; i<columns.size(); ++i){      但是 如果 有值为空  就不插入该字段   所以导致      以上的 字段数量和  后面 问号的数量不一致     所以改成了 现在这种方式
		sql.append(" values(");
...
//		return id;
	 ...
	 *             数据库异常
	 */
	public void save(BaseModel model) throws DataAccessException {
		if (model == null) {
			throw new DataAccessException(OBJECT_CAN_NOT_NULL);
		}
